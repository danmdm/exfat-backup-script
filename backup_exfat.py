...
def parse_args():
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  %(prog)s ~/Desktop /media/dan/stick --force-case\n"
            "  %(prog)s ~/Desktop /media/dan/stick --timeout-rsync 0  # fără timeout\n"
        ),
    ...
    parser.add_argument(
        '--force-case', action='store_true',
        help='Redenumește automat fișierele care se bat case-insensitive (exFAT)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--timeout-rsync', type=int, default=TIMEOUT_RSYNC_SEC,
        help=f'Timeout rsync în secunde (default: {TIMEOUT_RSYNC_SEC}). '
             f'0 = fără timeout.'
    )
...
def e_nume_invalid_exfat(nume):
    ...
    if _baza_nume(nume) in NUME_REZERVATE:
        return True
...
def curata_nume_exfat(nume):
    ...
    baza = _baza_nume(nume_curat)
    if baza in NUME_REZERVATE:
        nume_curat = f"_{nume_curat}"
...
# ==========================================
# COLIZIUNI CASE-INSENSITIVE (exFAT)
# ==========================================
def gaseste_coliziuni_case(radacina):
    coliziuni = []

    for root, dirs, files in walk_filtrat(radacina):
        intrari = dirs + files
        lower_map = {}
        for entry in intrari:
            lower_map.setdefault(entry.lower(), []).append(entry)

        for lower_name, entries in lower_map.items():
            if len(entries) > 1:
                coliziuni.append((root, sorted(entries)))

    coliziuni.sort(key=lambda x: x[0].count(os.sep), reverse=True)
    return coliziuni


def rezolva_coliziuni_case(coliziuni, is_dry_run, log_redenumiri, contoare):
    total = 0

    for folder, entries in coliziuni:
        if not os.path.isdir(folder):
            continue

        try:
            toate_lower = set(e.lower() for e in os.listdir(folder))
        except OSError:
            continue

        for entry in entries[1:]:
            cale_veche = os.path.join(folder, entry)
            if not os.path.exists(cale_veche):
                continue

            nume_baza, ext = os.path.splitext(entry)
            contor = 1
            while True:
                nume_nou = f"{nume_baza}_{contor}{ext}"
                if nume_nou.lower() not in toate_lower:
                    break
                contor += 1

            toate_lower.add(nume_nou.lower())
            cale_noua = os.path.join(folder, nume_nou)

            mesaj = f"[Case] {cale_veche} --> {nume_nou}"

            if is_dry_run:
                print(f"    {colorat('[SIMULARE]', 'galben')} {EMOJI['redenumire']} {cale_veche} → {nume_nou}")
            else:
                try:
                    os.rename(cale_veche, cale_noua)
                    print(f"    {EMOJI['redenumire']} {cale_veche} → {nume_nou}")
                except OSError as e:
                    print(colorat(f"    {EMOJI['eroare']} Eroare redenumire {cale_veche}: {e}", "rosu"))
                    continue

            log_redenumiri.append(mesaj)
            contoare["redenumiri"] += 1
            total += 1

    return total


def raporteaza_coliziuni_case(coliziuni):
    print()
    print(banner("COLIZIUNI CASE-INSENSITIVE", None, latime=70, culoare="rosu"))
    print(colorat("Pe exFAT (case-insensitive), aceste nume se bat cap în cap:", "rosu"))
    print()
    for folder, entries in coliziuni:
        print(f"  {colorat(folder + '/', 'cyan')}")
        for e in entries:
            print(f"    - {e}")
        print()
    print(colorat("Opțiuni:", "cyan"))
    print(colorat("  1. Redenumește manual una dintre ele pe laptop", "alb"))
    print(colorat("  2. Rulează cu --force-case pentru a le redenumi automat", "alb"))
...
# ==========================================
# JURNAL REDENUMIRI (local, mutat pe stick la final)
# ==========================================
def salveaza_jurnal_local(cale_jurnal_local, log_redenumiri):
    if not log_redenumiri:
        return
    try:
        with open(cale_jurnal_local, "w", encoding="utf-8") as f:
            f.write("Jurnal redenumiri\n")
            f.write("=" * 50 + "\n\n")
            f.write("\n".join(log_redenumiri))
            f.write("\n")
    except OSError:
...
def muta_jurnal_pe_stick(cale_jurnal_local, cale_jurnal_stick, log_redenumiri):
    if not log_redenumiri:
        return
    if not os.path.exists(cale_jurnal_local):
        return
    try:
        os.makedirs(os.path.dirname(cale_jurnal_stick), exist_ok=True)
        shutil.copy2(cale_jurnal_local, cale_jurnal_stick)
        os.remove(cale_jurnal_local)
    except OSError:
        pass
...
def ruleaza_rsync(sursa_abs, destinatia_folder, dir_modificate, is_dry_run,
                  verbose=False, timeout_rsync=TIMEOUT_RSYNC_SEC):
    if not is_dry_run:
        try:
    ...
    cmd = ["rsync", "-rtl", f"--modify-window={MODIFY_WINDOW}"]
    ...
    try:
        ...
        print(colorat(f"    {EMOJI['eroare']} Nu pot porni rsync: {e}", "rosu"))
        return 1, {}

    linii_output = []

    def citeste_stdout():
        """Citește linie cu linie, afișează live și acumulează pentru parsare."""
        try:
            for linie in process.stdout:
                print(linie, end="", flush=True)
                linii_output.append(linie)
        except Exception:
            pass
        finally:
            try:
    ...
    reader = threading.Thread(target=citeste_stdout, daemon=True)
    reader.start()

    try:
        if timeout_rsync > 0:
            cod_return = process.wait(timeout=timeout_rsync)
        else:
            cod_return = process.wait()
    except subprocess.TimeoutExpired:
        process.kill()
        reader.join(timeout=5)
        print(colorat(
            f"\n    {EMOJI['eroare']} rsync timeout după {timeout_rsync}s "
...
def main():
    ...
    if len(args.cai) < 2:
        ...
        print(f"  ./{script_name} ~/Desktop /media/dan/stick --force-case\n")
        pauza_finala()
        sys.exit(1)
    ...
    timeout_rsync = args.timeout_rsync
    ...
    cale_jurnal_local = f"/tmp/backup_{os.getpid()}.redenumiri.log"
    ...
    try:
        ...
        for sursa_abs in foldere_sursa:
            ...
            print(f"  {colorat('[1/5]', 'gri')} {EMOJI['redenumire']} Autocurățare nume...")
            ...
            if modificari == 0:
                ...
                if not is_dry_run:
                    salveaza_jurnal_local(cale_jurnal_local, log_redenumiri)

            print(f"  {colorat('[2/5]', 'gri')} {EMOJI['avert']} Verificare coliziuni case (exFAT)...")
            coliziuni_case = gaseste_coliziuni_case(sursa_abs)

            if coliziuni_case and not force_case:
                raporteaza_coliziuni_case(coliziuni_case)
                if log_redenumiri and not is_dry_run:
                    print()
                    print(colorat(f"{EMOJI['jurnal']} Pe laptop s-au aplicat deja {len(log_redenumiri)} redenumiri.", "galben"))
                    print(colorat(f"   Jurnal local: {cale_jurnal_local}", "galben"))
                trimite_notificare(
                    "Backup Eșuat",
                    f"{len(coliziuni_case)} coliziuni case-insensitive detectate",
                    iconita="dialog-error",
                )
                pauza_finala()
                sys.exit(1)

            if coliziuni_case and force_case:
                print(f"         {colorat(f'{len(coliziuni_case)} coliziuni detectate, se rezolvă...', 'galben')}")
                rezolva_coliziuni_case(coliziuni_case, is_dry_run, log_redenumiri, contoare)
                if not is_dry_run:
                    salveaza_jurnal_local(cale_jurnal_local, log_redenumiri)
            else:
                print(f"         {colorat('Nicio coliziune case-insensitive.', 'gri')}")

            print(f"  {colorat('[3/5]', 'gri')} {EMOJI['index']} Indexare fișiere...")
            fisiere_exacte, dupa_hash, nr_fisiere = indexeaza_sursa(sursa_abs)
            print(f"         {colorat(f'{nr_fisiere} fișiere indexate', 'gri')}")

            print(f"  {colorat('[4/5]', 'gri')} {EMOJI['scut']} Verificare siguranță ștergeri...")
            ...
            print(f"  {colorat('[5/5]', 'gri')} {EMOJI['spatiu']} Spațiu necesar: "
        ...
        if free < necesar_total:
            ...
            if log_redenumiri and not is_dry_run:
                print()
                print(colorat(f"{EMOJI['jurnal']} Pe laptop s-au aplicat {len(log_redenumiri)} redenumiri.", "galben"))
                print(colorat(f"   Jurnal local: {cale_jurnal_local}", "galben"))
            else:
                print(colorat("Nicio modificare nu a fost aplicată pe HDD.", "galben"))
            trimite_notificare("Backup Eșuat", msg, iconita="dialog-error")
        ...
        for idx_exec, entry in enumerate(plan, 1):
            ...
            cod, statistici = ruleaza_rsync(
                sursa_abs, destinatia_folder, dir_modificate, is_dry_run,
                verbose, timeout_rsync,
            )
        ...
        if log_redenumiri and not is_dry_run:
            muta_jurnal_pe_stick(cale_jurnal_local, cale_log_redenumiri, log_redenumiri)
